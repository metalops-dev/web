...
I set this up recently and surprised me how smoothly it worked.

- **Proxy:** Traefik 3.7.13
...
<Callout type="note" title="PostgreSQL 17">
  Since PostgreSQL 17, `sslmode=direct` changes the way connections are handled
  by eliminating the initial `SSLRequest` round-trip, which reduces latency.
</Callout>
...
<Callout type="warning" title="Network boundary">
  Traffic between Traefik and Postgres is unencrypted. That is "fine" when both
  are on the same internal Docker network. If you're running a multi-host
  cluster where Traefik and Postgres live on different hosts, the traffic should
  be encrypted on that side too.
</Callout>
...
This works well on a single VPS. For multi-host architecture, encrypt the connection between Traefik and PostgreSQL as well.
